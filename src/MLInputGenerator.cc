...
namespace MLInputGenerator {
  static map<string, function<double(const Jet*)> > jetInputs;
  static map<string, function<double(const Track*, const Jet* jet)> > trackInputs;
  static map<string, function<double(const Neutral*, const Jet* jet)> > neutralInputs;

  void init(){

    // jet inputs
    jetInputs["jet_px"] = [](const Jet* jet){ return jet->Px(); };
    jetInputs["jet_py"] = [](const Jet* jet){ return jet->Py(); };
    jetInputs["jet_pz"] = [](const Jet* jet){ return jet->Pz(); };
    jetInputs["jet_mass"] = [](const Jet* jet){ return jet->M(); };
    jetInputs["jet_ntracks"] = [](const Jet* jet){ return jet->getAllTracks().size(); };
    jetInputs["jet_nneutrals"] = [](const Jet* jet){ return jet->getNeutrals().size(); };
    jetInputs["jet_phi"] = [](const Jet* jet){ return jet->Phi(); };
    jetInputs["jet_theta"] = [](const Jet* jet){ return jet->Theta(); };

    // track inputs
    trackInputs["tr_px"] = [](const Track* tr, const Jet*){ return tr->Px(); };
    trackInputs["tr_py"] = [](const Track* tr, const Jet*){ return tr->Py(); };
    trackInputs["tr_pz"] = [](const Track* tr, const Jet*){ return tr->Pz(); };
    trackInputs["tr_e"] = [](const Track* tr, const Jet*){ return tr->E(); };
    trackInputs["tr_efrac"] = [](const Track* tr, const Jet* jet){ return tr->E() / jet->E(); };
    trackInputs["tr_erel_log"] = [](const Track* tr, const Jet* jet){ return log10( tr->E() / jet->E() ); };
    trackInputs["tr_thetarel"] = [](const Track* tr, const Jet* jet){
      auto part = tr->Vect();
      auto jet3 = jet->Vect();
      part.RotateZ(-jet3.Phi());
      part.RotateY(-jet3.Theta());
      return part.Theta();
    };
    trackInputs["tr_phirel"] = [](const Track* tr, const Jet* jet){
      auto part = tr->Vect();
      auto jet3 = jet->Vect();
      part.RotateZ(-jet3.Phi());
      part.RotateY(-jet3.Theta());
    ...
    trackInputs["tr_thetarel_ilc"] = [](const Track* tr, const Jet* jet){ return tr->Theta() - jet->Theta(); };
    trackInputs["tr_phirel_ilc"] = [](const Track* tr, const Jet* jet){
      auto ret = tr->Phi() - jet->Phi();
      if(ret < -TMath::Pi()) ret += TMath::Pi() * 2;
      if(ret > TMath::Pi()) ret -= TMath::Pi() * 2;
      return ret;
    };
    trackInputs["tr_dptdpt"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::omom]; };
    trackInputs["tr_detadeta"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::tdtd]; };
    trackInputs["tr_dphidphi"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::phph]; };
    trackInputs["tr_dxydxy"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::d0d0]; };
    trackInputs["tr_dzdz"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::z0z0]; };
    trackInputs["tr_dxydz"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::d0z0]; };
    trackInputs["tr_dphidxy"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::d0ph]; };
    trackInputs["tr_dlambdadz"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::z0td]; };
    trackInputs["tr_dxyc"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::d0om]; };
    trackInputs["tr_dxyctgtheta"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::d0td]; };
    trackInputs["tr_phic"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::phom]; };
    trackInputs["tr_phidz"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::z0ph]; };
    trackInputs["tr_phictgtheta"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::phtd]; };
    trackInputs["tr_cdz"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::z0om]; };
    trackInputs["tr_cctgtheta"] = [](const Track* tr, const Jet*){ return tr->getCovMatrix()[tpar::omtd]; };
  }
...
float MLInputGenerator::calc_dxy(float D0_wrt0, float Z0_wrt0, float phi0_wrt0, TVector3 p, TVector3 privtx, int charge){
  double Bz = 3.5;
...
float MLInputGenerator::calc_dz(float D0_wrt0, float Z0_wrt0, float phi0_wrt0, TVector3 p, TVector3 privtx, int charge){
  double Bz = 3.5;
  double cSpeed = 2.99792458e8 * 1e-9;
...
float MLInputGenerator::calc_sip2d(float D0, float phi0, float jetpx, float jetpy)
{
...
float MLInputGenerator::calc_sip3d(float D0, float Z0, float phi0, TVector3 p_jet)
{
  TVector3 d(-D0 * TMath::Sin(phi0), D0 * TMath::Cos(phi0), Z0);
  return TMath::Sign(1, d * p_jet) * fabs(sqrt(D0 * D0 + Z0 * Z0));
}

float MLInputGenerator::calc_jetDist(float D0, float Z0, float phi0, TVector3 p_ct, TVector3 p_jet)
{
  TVector3 d(-D0 * TMath::Sin(phi0), D0 * TMath::Cos(phi0), Z0);
...
void MLInputGenerator::calc_thetaphi(TVector3 jet, TVector3 part, float &theta, float &phi){
